      ...
      <div class="overflow-x-auto bg-white shadow rounded-lg">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-200 text-sm">
            <tr v-for="(task, index) in tasksForSelectedDate" :key="index" class="hover:bg-gray-50">
              <td class="px-4 py-2">{{ task.hora }} - {{ task.horaFin }}</td>
              <td class="px-4 py-2">{{ task.tema }}</td>
              <td class="px-4 py-2">{{ task.descripcion }}</td>
              <td class="px-4 py-2">{{ task.actividad }}</td>
              <td class="px-4 py-2">{{ task.lugar }}</td>
              <td class="px-4 py-2">{{ grupos.find(g => g.id === task.grupoId)?.nombre || 'Sin grupo' }}</td>
              <td class="px-4 py-2 flex justify-center gap-2">
                <button
                  @click="viewTaskDetails(task)"
                  class="bg-indigo-500 hover:bg-indigo-600 text-white rounded px-2 py-1"
                  title="Ver detalles"
                >
            ...
            <tr v-if="tasksForSelectedDate.length === 0">
              <td colspan="7" class="text-center px-4 py-4 text-gray-500">No hay actividades registradas</td>
            </tr>
          </tbody>
        </table>
      </div>
